...
informative:
  ...
  IFIP18:
    author:
     ...
     - ins: P. Camarillo
     - ins: C. Filsfils
    title: SEgment Routing Aware Firewall For Service Function Chaining scenarios
    seriesinfo: IFIP Networking conference
    date: May 2018
  IPTABLES:
    title: iptables-1.6.2 changes
    date: February 2018
    target: https://netfilter.org/projects/iptables/files/changes-iptables-1.6.2.txt
  NFTABLES:
    title: nftables-0.8.4 changes
    date: May 2018
    target: https://netfilter.org/projects/nftables/files/changes-nftables-0.8.4.txt
  SNORT:
    title: SR-Snort
    date: March 2018
    target: https://github.com/SRouting/SR-Snort
...
...
Examples of SR-aware services are provided in section {{sec-implem-aware}}.
...
An SR-unaware service is associated with a service segment instantiated on the SR proxy, which is used to steer traffic through the service. {{sec-proxies}} describes several SR proxy behaviors to handle the encapsulation headers and SR information under various circumstances.


# SR Service Policies

An SR service policy is an SR policy, as defined in {{RFC9256}}, that includes at least one service. This service is represented in the SID-list by its associated service SID. In case the policy should include several services, the service traversal order is indicated by the relative position of each service SID in the SID-list. Using the mechanisms described in {{RFC9256}}, it is possible to load balance the traffic over several services, or instances of the same service, by associating with the SR service policy a weighted set of SID-lists, each containing a possible sequence of service SIDs to be traversed. Similarly, several candidate paths can be specified for the SR service policy, each with its own set of SID-lists, for resiliency purposes.
...
+----+----+             +---------+             +----+-----+
     ...
     +-----------------------------------------------+
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #fig-policy-srv6 title="Packet walk in an SRv6 network"}
...
The "Inner pkt" on {{fig-policy-srv6}} represents the SRv6 payload, which may be an encapsulated IP packet, an Ethernet frame or a transport-layer payload, for example.


# SR Proxy Behaviors {#sec-proxies}
...
---------->|          SR proxy          |---------->
           ...
           +----------------------------+
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #fig-proxy title="Generic SR proxy"}
...
+----------------+----------------------+-----+-----+-----+-----+
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #fig-proxy-sum title="SR proxy summary"}
...
When processing an IPv6 packet matching a FIB entry locally instantiated as an
...
S20. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-eth-sid title="SID processing for SRv6 static proxy
(Ethernet)"}

S15: 143 (Ethernet) refers to the value assigned by IANA for "Ethernet" in the
...
S05. Submit the frame to the Ethernet module for transmission via
     interface IFACE-OUT.
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-eth-ulh title="Upper-layer header processing for SRv6
static proxy (Ethernet)"}
...
S06. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-eth-inbound title="Inbound policy for SRv6 static proxy
(Ethernet)"}

S04: CACHE.LIST\[0\] represents the first entry in CACHE.LIST. Unless a local
...
header set to 143 (Ethernet).

#### Static Proxy for Inner Type IPv4

When processing an IPv6 packet matching a FIB entry locally instantiated as an
SRv6 static proxy SID for IPv4 traffic, the following pseudocode is executed.
...
S20. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-ipv4-sid title="SID processing for SRv6 static proxy
(IPv4)"}
...
S05. Submit the packet to the IPv4 module for transmission on
     interface IFACE-OUT via NH-ADDR.
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-ipv4-ulh title="Upper-layer header processing for SRv6
static proxy (IPv4)"}
...
S06. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-ipv4-inbound title="Inbound policy for SRv6 static proxy
(IPv4)"}
...
values should be set of the length of CACHE.LIST minus 1. If CACHE.LIST contains
a single entry, the SRH can be omitted and the Next Header field of the IPv6
header set to 4 (IPv4).

#### Static Proxy for Inner Type IPv6

When processing an IPv6 packet matching a FIB entry locally instantiated as an
SRv6 static proxy SID for IPv6 traffic, the following pseudocode is executed.
...
S20. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-ipv6-sid title="SID processing for SRv6 static proxy
(IPv6)"}
...
S05. Submit the packet to the IPv6 module for transmission on
     interface IFACE-OUT via NH-ADDR.
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-ipv6-ulh title="Upper-layer header processing for SRv6
static proxy (IPv6)"}
...
S06. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-static-srv6-ipv6-inbound title="Inbound policy for SRv6 static proxy
(IPv6)"}
...
IPv6 header set to 41 (IPv6).

## Dynamic SR Proxy {#sec-proxies-dynamic}
...
{{code-static-srv6-eth-sid}}, {{code-static-srv6-ipv4-sid}} and
{{code-static-srv6-ipv6-sid}}, respectively for Ethernet, IPv4 and IPv6 traffic,
is executed with the following addition between lines S17 and S18.
...
(S18.     Perform IPv6 decapsulation...)
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-dynamic-srv6-sid title="SID processing for SRv6 dynamic proxy"}

An implementation may optimize the caching procedure by copying information into
...
restores the IPv6 encapsulation from the cache entry.

When processing the Upper-layer header of a packet matching a FIB entry locally
instantiated as an SRv6 dynamic proxy SID, process the packet as per
{{RFC8986}} Section 4.1.1.

When processing an Ethernet frame, an IPv4 packet or an IPv6 packet received on
the interface IFACE-IN and with a destination address that does not match any
address of IFACE-IN, the same pseudocode as in {{code-static-srv6-eth-inbound}},
{{code-static-srv6-ipv4-inbound}} or {{code-static-srv6-ipv6-inbound}},
...
Masquerading: When processing an IPv6 packet matching a FIB entry locally
...
S16. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-masquerading-sid title="SID processing for SRv6 masquerading proxy"}

When processing the Upper-layer header of a packet matching a FIB entry locally
instantiated as an SRv6 masquerading proxy SID, process the packet as per
{{RFC8986}} Section 4.1.1.

De-masquerading: When processing an IPv6 packet received on the interface
IFACE-IN and with a destination address that does not match any address of
IFACE-IN, the following pseudocode is executed.
...
S14. }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
{: #code-masquerading-inbound title="Inbound policy for SRv6 masquerading proxy"}

### Destination NAT Flavor {#sec-proxies-am-nat}

Services modifying the destination address in the packets they process, such as
NATs, can be supported by reporting the updated Destination Address back into
the Segment List field of the SRH.

The Destination NAT flavor of the SRv6 masquerading proxy is enabled by adding
the following instruction between lines S09 and S10 of the de-masquerading
pseudocode in {{code-masquerading-inbound}}.
...
### Caching Flavor {#sec-proxies-am-cache}

Services generating packets or acting as endpoints for transport connections can be supported by adding a dynamic caching mechanism similar to the one described in {{sec-proxies-dynamic}}.

The caching flavor of the SRv6 masquerading proxy is enabled by:

- Adding the following instruction between lines S14 and S15 of the masquerading
pseudocode in {{code-masquerading-sid}}.
...
- Updating the de-masquerading pseudocode such that, in addition to the SRH
processing in {{code-masquerading-inbound}}, the following pseudocode is
...
## SR-Aware Services {#sec-implem-aware}
